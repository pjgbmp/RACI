...
/* ── Inputs dentro de expanders del sidebar → texto negro legible ── */
...
if not st.session_state["active_group_id"]:
    ...
    st.markdown(
        ...
        f'<div style="color:white;font-size:1.8rem;font-weight:700;margin-bottom:8px">Bienvenido/a, {_fn_w}!</div>'
    ...
    _onboard_cols = st.columns(4)
    _onboard_steps = [
        ("1️⃣", "Crea o únete a un grupo", "Tu espacio de trabajo compartido."),
        ("2️⃣", "Crea un proyecto", "Agrupa tareas bajo un objetivo común."),
        ("3️⃣", "Agrega tareas con RACI", "Define quién ejecuta, aprueba y necesita saber."),
        ("4️⃣", "Sigue el progreso", "Tablero Kanban y métricas en tiempo real."),
    ]
    for col, (num, title_s, desc) in zip(_onboard_cols, _onboard_steps):
        with col:
            st.markdown(
                f'<div style="background:white;border:1px solid #E8EDF5;border-radius:12px;'
                f'padding:16px;text-align:center;height:140px;">'
...
_MENU_DESC = {
    "Resumen": "Dashboard · KPIs · riesgos",
    "Proyectos": "Crear y gestionar proyectos",
    "Tablero": "Kanban: ver y mover tareas",
    "Tarea (detalle)": "Comentar · cambiar estado",
    "Plantillas": "Reutilizar estructuras",
    "Export": "CSV · Excel · Calendario ICS",
    "Gobernanza": "Permisos y reglas del grupo",
    "Reportes": "Reporte semanal del equipo",
    "Ayuda": "Guía completa · FAQ",
    "Admin": "Panel administrador global",
}
_desc_nav = _MENU_DESC.get(menu, "")
if _desc_nav:
    st.sidebar.markdown(
        f'<div style="background:rgba(255,255,255,0.06);border-radius:8px;padding:6px 10px;'
        f'font-size:0.76rem;color:#8A9BC8;margin-top:-4px;margin-bottom:6px">{_desc_nav}</div>',
        unsafe_allow_html=True,
    )
...
def _empty_state(icon: str, title: str, body: str, cta: str = "", cta_page: str = ""):
    """Empty state accionable: contexto claro + guía + CTA."""
    btn_html = (
        f'<div style="margin-top:14px">'
        f'<span style="display:inline-block;background:#4C8BF5;color:white;'
        f'border-radius:8px;padding:8px 20px;font-weight:600;font-size:0.92rem">'
        f'→ {cta}</span></div>'
    ) if cta else ""
    st.markdown(
    ...
    if cta and cta_page:
        if st.button(f"→ {cta}", key=f"_es_{cta_page}_{title[:10]}", type="primary"):
            set_menu(cta_page)
...
def _step_indicator(steps: list, current: int):
    ...
    for i, (ico, lbl) in enumerate(steps):
        ...
        active = i == current
        bg = "#4C8BF5" if active else ("#48BB78" if done else "#E8EDF5")
...
if menu == "Resumen":
    ...
    if not pmap:
        ...
        _step_indicator(
            ...
            current=0 if _members_count <= 1 else 1,
        )
        _empty_state(
            "📁",
            "Aún no tienes proyectos en este grupo",
            "Los proyectos organizan las tareas de tu equipo. Crea el primero para asignar trabajo, definir responsabilidades y hacer seguimiento.",
    ...
    if kpi_df.empty:
        _empty_state(
            ...
            "Los KPIs se calculan a partir de tareas con RACI asignado. Crea tareas, asigna ejecutores (R) y comienza a trabajar para ver las métricas.",
            cta="Ir al Tablero",
            cta_page="Tablero",
        )
    else:
...
# =============================================================================
    ...
    if not projs:
        ...
        _inline_tip("El límite WIP (tareas en curso por persona) se configura al crear el proyecto. El valor recomendado es 3.")
    else:
    ...
    if not has_permission(GROUP_ID, uid, "project_create"):
        st.markdown(
            ...
            '🔒 No tienes permiso para crear proyectos. Pide al líder que te otorgue el permiso '
            '<code>project_create</code> en la pestaña de Gobernanza.</div>',
            unsafe_allow_html=True,
        )
    else:
        _inline_tip("Un proyecto agrupa tareas de un mismo objetivo. El límite WIP controla cuántas tareas puede tener cada persona 'En curso' (recomendado: 3).")
        with st.form("new_project"):
            ...
            wip = st.number_input(
                ...
                help="WIP = Work In Progress. Si alguien ya tiene este número de tareas en curso, no podrá tomar más hasta completar alguna."
            )
...
# =============================================================================
    ...
    if not projs:
        ...
        _empty_state(
            "📌", "El tablero espera su primer proyecto",
            "El Tablero Kanban organiza las tareas en columnas: Por hacer → En curso → En aprobación → Completado. Para usarlo necesitas primero crear un proyecto.",
    ...
    with f1:
        q = st.text_input("🔍 Buscar tarea", placeholder="Título, descripción o DoD...", help="Busca en título, descripción y criterio de aceptación.")
    with f2:
        f_status = st.multiselect("Estado", ["todo", "doing", "blocked", "awaiting_approval", "done"],
                                  default=["todo", "doing", "blocked", "awaiting_approval"],
                                  help="Por defecto oculta las completadas para no saturar la vista.")
    with f3:
        f_priority = st.multiselect("Prioridad", ["urgente", "alta", "media", "baja"],
                                    default=["urgente", "alta", "media", "baja"],
                                    help="Desactiva 'baja' para enfocarte en lo importante.")
    with f4:
        only_mine = st.checkbox("Solo mis tareas", value=False, help="Muestra solo tareas donde eres Ejecutor (R) o Dueño (A).")

    tag_filter = st.multiselect("Filtrar por tags/componentes", options=list(tag_opts.keys()), help="Útil para ver solo tareas de un sprint o área específica.")
    c_ov, c_bl = st.columns(2)
    with c_ov:
        only_overdue = st.checkbox("🔴 Solo vencidas", value=False, help="Tareas cuya fecha límite ya pasó y siguen abiertas.")
    with c_bl:
        only_blocked = st.checkbox("⛔ Solo bloqueadas", value=False, help="Muestra solo tareas en estado blocked.")
    ...
    for s, col in zip(labels.keys(), cols):
        with col:
            ...
            if not by_status[s]:
                _col_hints = {
                    "todo": ("📋", "Sin pendientes", "Crea una tarea más abajo ↓"),
                    "doing": ("🔵", "Nadie en curso", "Mueve una tarea a 'En curso' desde su detalle"),
                    "blocked": ("✅", "Sin bloqueos", "¡Todo fluye! No hay tareas detenidas"),
                    "awaiting_approval": ("🟡", "Sin aprobaciones", "Las tareas pendientes de revisión aparecen aquí"),
                    "done": ("🎉", "Sin completadas", "Las tareas cerradas esta semana aparecerán aquí"),
                }
                ico_h, ttl_h, bdy_h = _col_hints.get(s, ("📭", "Vacío", ""))
                st.markdown(
                    f'<div style="text-align:center;padding:20px 8px;color:#A0AEC0;">'
    ...
    if not has_permission(GROUP_ID, uid, "task_create"):
        ...
        _inline_tip("Obligatorios: Título ✱, Criterio de aceptación (DoD) ✱, un Ejecutor (R) ✱ y un Dueño (A) ✱. El resto es opcional pero recomendado.")
        with st.form("new_task"):
            title = st.text_input("Título ✱", placeholder="ej: Diseñar pantalla de login", help="Nombre claro y específico. Evita títulos genéricos como 'Revisar'.")
            desc = st.text_area("Descripción", height=80, placeholder="Contexto, recursos, links relevantes...", help="Info adicional para que el ejecutor entienda el trabajo sin preguntas.")
            dod = st.text_area(
                "Criterio de aceptación — DoD ✱", height=80,
                placeholder="ej: La pantalla está aprobada por diseño, funciona en mobile y pasa QA.",
                help="¿Cómo sabremos que esta tarea está DONE? Sin DoD no hay forma objetiva de cerrar la tarea."
            )
            c1, c2, c3, c4 = st.columns(4)
            with c1:
                priority = st.selectbox("Prioridad", ["urgente", "alta", "media", "baja"], index=2,
                    help="🔴 Urgente: bloquea todo · 🟠 Alta: esta semana · 🔵 Media: próx. 2 semanas · 🟢 Baja: cuando haya tiempo")
            with c2:
                use_start = st.checkbox("Usar fecha de inicio", value=False, help="Actívalo si la tarea no puede comenzar antes de una fecha específica.")
                start_d = st.date_input("Inicio", value=date.today()) if use_start else None
            with c3:
                target_d = st.date_input("Objetivo SLA", value=date.today() + timedelta(days=7), help="Meta interna de entrega. Tu SLA de equipo, no el deadline externo.")
            with c4:
                due_d = st.date_input("Vencimiento ✱", value=date.today() + timedelta(days=10), help="Fecha límite real. Superarla marca la tarea como vencida.")
            requires_approval = st.checkbox("Requiere aprobación del Dueño (A) para cerrarse", value=True,
                help="Al marcar como done, pasa a 'En aprobación' hasta que el Dueño apruebe o rechace.")
            st.markdown("##### 👥 Responsabilidad RACI ✱")
            st.caption("Define quién hace qué. Se requiere al menos un Dueño (A) y un Ejecutor (R).")
            if m_opts:
                A = st.selectbox("🔷 Dueño (A) — responsable final único", options=list(m_opts.keys()),
                    help="Accountable: aprueba el cierre, toma decisiones finales. Solo puede haber UNO.")
                R = st.multiselect("🔧 Ejecutores (R) — quienes hacen el trabajo ✱", options=list(m_opts.keys()),
                    help="Responsible: ejecutan la tarea. Pueden ser varios. Solo ellos cambian el estado.")
                C = st.multiselect("💬 Consultados (C) — su opinión importa", options=list(m_opts.keys()),
                    help="Consulted: se les pregunta durante la ejecución. Reciben notificación al crearse.")
                I = st.multiselect("📣 Informados (I) — necesitan saber", options=list(m_opts.keys()),
                    help="Informed: solo reciben información del avance. No toman decisiones.")
            else:
                A = None; R = []; C = []; I = []
            st.markdown("##### 🏷️ Clasificación")
            selected_tags = st.multiselect("Tags (etiquetas libres)", options=tag_names,
                help="Se crean automáticamente si no existen. Úsalos para filtrar en el Tablero.")
            selected_component = st.selectbox("Componente", options=["(ninguno)"] + comp_names,
                help="Categoría técnica o área. ej: API, UI, Base de datos.")
            existing = list_tasks_by_project(pid, limit=300)
            dep_map = {f"#{x['id']} {x['title']}": int(x["id"]) for x in existing}
            deps_sel = st.multiselect("🔗 Bloqueada por (dependencias)", options=list(dep_map.keys()),
                help="Tareas que deben completarse antes. La tarea quedará bloqueada automáticamente si siguen abiertas.")
            send_email_now = st.checkbox("📧 Notificar por correo al Dueño y Ejecutores", value=False,
                help="Requiere email en el perfil del usuario y servicio de correo activo.")
...
# =============================================================================
    ...
    if not tasks:
        _empty_state(
            ...
            "Las tareas representan el trabajo concreto de tu equipo. Créalas desde el Tablero y asigna responsabilidades con RACI.",
    ...
    if can_change_status(GROUP_ID, task_id, uid):
        _STATUS_FLOW = {
            "todo": "📋 Por hacer → muévela a **En curso** cuando empieces a trabajar.",
            "doing": "🔵 En curso → marca como **Completada** al terminar, o **Bloqueada** si hay un impedimento.",
            "blocked": "🔴 Bloqueada → resuelve el impedimento y vuelve a **En curso**.",
            "awaiting_approval": "🟡 En aprobación → el **Dueño (A)** debe aprobar o rechazar desde esta misma pantalla.",
        ...
        new_status = st.selectbox(
            ...
            help="Flujo: Por hacer → En curso → (En aprobación) → Completada.",
            format_func=lambda s: {
                "todo": "📋 Por hacer", "doing": "🔵 En curso",
                "blocked": "🔴 Bloqueada", "awaiting_approval": "🟡 En aprobación",
                "done": "✅ Completada",
            }.get(s, s)
        )
...
# =============================================================================
    ...
    if df_done.empty:
        _empty_state("✅", "No hay tareas completadas en esta semana", "Las tareas cerradas durante el período seleccionado aparecerán aquí. Prueba con otra semana o revisa el Tablero.")
    else:
